...
exports.seedLargeDatabase = seedLargeDatabase;
const mongoose_1 = __importDefault(require("mongoose"));
const model_1 = require("../modules/users/model");
const model_2 = require("../modules/vehicles/model");
const model_3 = require("../modules/drivers/model");
const model_4 = require("../modules/trips/model");
const model_5 = require("../modules/maintenance/model");
const model_6 = require("../modules/fuel/model");
const model_7 = require("../modules/expenses/model");
const model_8 = require("../modules/notifications/model");
const model_9 = require("../modules/activities/model");
const model_10 = require("../modules/reports/model");
const VEHICLE_MAKES = [
...
async function seedLargeDatabase() {
    ...
    const userId = new mongoose_1.default.Types.ObjectId("60c72b2f9b1d8b3a7c8c8c8d");
    console.log("Starting full database seeding...");
    // 1. CLEAR COLLECTIONS
    await model_1.UserModel.deleteMany({});
    await model_2.VehicleModel.deleteMany({});
    await model_3.DriverModel.deleteMany({});
    await model_4.TripModel.deleteMany({});
    await model_5.MaintenanceModel.deleteMany({});
    await model_6.FuelLogModel.deleteMany({});
    await model_7.ExpenseModel.deleteMany({});
    await model_8.NotificationModel.deleteMany({});
    ...
    await model_2.VehicleModel.insertMany(vehiclesList);
    ...
    await model_3.DriverModel.insertMany(driversList);
    ...
    await model_4.TripModel.insertMany(tripsList);
    ...
    await model_5.MaintenanceModel.insertMany(maintenanceList);
    ...
    await model_6.FuelLogModel.insertMany(fuelList);
    ...
    for (let i = 31; i <= 70; i++) {
        ...
        if (cat === 'Salaries')
            amt = randomRange(1200, 3000);
        if (cat === 'Insurance')
            amt = randomRange(450, 1500);
        expensesList.push({
            id: `E${String(i).padStart(3, '0')}`,
            category: cat,
            amount: amt,
    ...
    await model_7.ExpenseModel.insertMany(expensesList);
    ...
    await model_8.NotificationModel.insertMany(notificationsList);
    ...
    await model_9.ActivityModel.insertMany(activitiesList);
    ...
    const dbVehicles = await model_2.VehicleModel.find({ organization_id: orgId });
    for (const vehicle of dbVehicles) {
        const vId = vehicle.id;
        // Sum completed trips distance & revenue & fuel
        const completedTrips = await model_4.TripModel.find({ vehicleId: vId, status: "Completed" });
        ...
        // Sum fuel logs
        const fuelLogs = await model_6.FuelLogModel.find({ vehicleId: vId });
        const totalFuelCost = fuelLogs.reduce((sum, f) => sum + f.cost, 0);
        // Sum maintenance logs
        const maintenanceLogs = await model_5.MaintenanceModel.find({ vehicleId: vId, status: "Completed" });
        const totalMaintCost = maintenanceLogs.reduce((sum, m) => sum + m.cost, 0);
        await model_2.VehicleModel.updateOne({ _id: vehicle._id }, {
            $set: {
